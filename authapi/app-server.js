...
APP.use(
  CORS({
    origin: function (origin, callback) {
      ...
      if (!origin)
        return callback(
          CONSTANT_FILE_OBJECT.APP_CONSTANT.NULL,
          CONSTANT_FILE_OBJECT.APP_CONSTANT.TRUE
        );
      if (
        ALLOWED_ORIGINS.indexOf(origin) ===
        CONSTANT_FILE_OBJECT.APP_CONSTANT.MINUS_ONE
      ) {
        let message =
          "The CORS policy for RiskTrac site does not allow access from the specified Origin. : Origin : " +
          origin;
        return callback(
          new Error(message),
          CONSTANT_FILE_OBJECT.APP_CONSTANT.FALSE
        );
      }
      return callback(
        CONSTANT_FILE_OBJECT.APP_CONSTANT.NULL,
