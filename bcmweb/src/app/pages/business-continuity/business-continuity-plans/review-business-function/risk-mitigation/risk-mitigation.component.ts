...
export class RiskMitigationComponent {
  ...
  ngOnInit() {
    this.businessContinuityService.TableRM = new MatTableDataSource();
  ...
  patchValue(data: any) {
    setTimeout(() => {
      this.businessContinuityService.TableRM = new MatTableDataSource(this.businessContinuityService.addIndex(data, true))
    }, 1000);
  }

  cancel(Critical: any) {
    this.blockEdit = false;
    this.businessContinuityService.TableRM.data.forEach((x: any) => {
      x.isEdit = false;
    });

    if (this.controlMode == "Edit") {
      this.businessContinuityService.TableRM._updateChangeSubscription();
    } else {
      const index = this.businessContinuityService.TableRM.data.indexOf(Critical.index);
      this.businessContinuityService.TableRM.data.splice(index, 1);
      this.businessContinuityService.TableRM._updateChangeSubscription();
    }
  }
  ...
  save(data: any) {
    ...
    this.businessContinuityService.TableRM.data =
      this.businessContinuityService.TableRM.data.map((x: any) =>
        x.Index === data.Index
          ? {
            ...x,
  ...
  addRisk() {
    ...
    this.businessContinuityService.TableRM.data.push({ "Index": this.businessContinuityService.TableRM.data.length + 1, "PotentialData": null, "AffectedProcess": '', 'Impact': '', 'Likelihood': '', 'Risk': '', 'Contingency': '', 'Treatment': '', "isEdit": true });
    this.businessContinuityService.TableRM._updateChangeSubscription();
    this.riskMitigationFlag = true
    this.riskRating = ""
  }
  ...
  onSelectLikelihood(id: any) {
    ...
    this.businessContinuityService.getOverallRiskRating(data).subscribe(res => {
      if (res.success == 1) {
        ...
        this.riskRating = res.result.OverAllRiskRating[0].OverallInherentRiskRating
      }
    })
    this.riskMitigationFlag = true
  }
  ...
  onSubmit() {
    let output = this.modifyData(this.businessContinuityService.TableRM.data, Number(localStorage.getItem("BusinessContinuityPlanID")));
  ...
  isAnyTableInvalid(): boolean {
    let allIDsIncluded = this.businessContinuityService.TableRM.data.some(entry => entry.PotentialData !== "" && entry.AffectedProcess !== "" && entry.Impact !== "" && entry.Likelihood !== "" && entry.Risk !== "" && entry.Contingency !== "" && entry.Treatment !== "");
