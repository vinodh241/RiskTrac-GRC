...
export class ResourceRequirementsComponent {
  ...
  isSaveDisable() {
    ...
    if (vitalFields) {
      if ((vitalFields.RecordType?.length > 0) && (vitalFields.MediaType?.length > 0) && (vitalFields.AlternateSource?.length > 0)) {
        return true;
      }
    }
    return false;
  ...
  isSaved() {
    ...
    if (criticalFields) {
      if ((criticalFields.Equipment?.length > 0) && (criticalFields.Description?.length > 0) && (Number(criticalFields.TotalCount) > 0) && (Number(criticalFields.MinimumCount) > 0)) {
        return true;
      }
    }
    return false;
