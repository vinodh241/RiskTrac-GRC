...
@Component({
  selector: 'app-initiate-bia-exercise',
  templateUrl: './initiate-bia-exercise.component.html',
  styleUrls: ['./initiate-bia-exercise.component.scss']
})
export class InitiateBiaExerciseComponent {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<InitiateBiaExerciseComponent>
  ) {
      ...
      if (result) {
        this.service.initiateBCP(item.BusinessFunctionID).subscribe(res => {
          if (res.success == 1) {
            ...
            this.saveSuccess("BIA Exercise initiated successfully");
