...
export class BusinessFunctionProfileComponent {
  ...
  @Output() checkBusinessProfileDataChanges = new EventEmitter<any>();
  ...
  payloadData: any;
  savedData:any = [];
  CustomersRespData:any[] =[];
  CriticalBusinessActivitiesRespData : any [] =[];
  ...
  ngOnInit() {
    ...
    localStorage.removeItem('criticalProcess');
    ...
    this.businessContinuityService.processBusinessMaster.subscribe((value) => {
      if (value) {
        ...
        this.savedData                = this.businessContinuityService.processBusinessDetails?.BusinessFunctionProfileDetails || [];
        ...
        let customers                 = this.savedData[0]?.Customers;
        let criticalProcess           =  this.savedData[0]?.CriticalBusinessActivities;
  ...
  addCriticalBusiness() {
    console.log('✌️this.CustomersRespData --->', this.CustomersRespData);
    console.log('✌️this.savedData[0].Customers --->', this.savedData[0].Customers);

    const CustomersCopy = [...this.savedData[0].Customers];
    this.businessFunctionFlag = true;
    this.criticalData = "";
    this.affiliationStatus = "";
    ...
    this.businessContinuityService.TableCBP._updateChangeSubscription();
    this.CustomersRespData = CustomersCopy;
  }
  ...
  async compareProfilingQuestionsData() {

    this.savedData[0].Customers = this.CustomersRespData;
    let profilingQuestions = this.parseInputJSON(this.profilingForm.value);

    const updatedData = this.businessContinuityService.TableCBP.data.map((item: any) => {
        let IsInternal = item.Affiliate === "Internal" || item.affiliationDataStatus === "Internal";
        return {
            CustomerName: item.Description,
            IsInternal: IsInternal,
            CustomerID: item.CustomerID || item.activityID || null
        };
    });

    const business = this.businessContinuityService.TableCB.data.map(item => ({
        ActivityName: item.Description,
        ActivityID: item.ActivityID || null
    }));

    let data = {
        "BusinessFunctionProfileID": Number(this.allViewDetails.BusinessFunctionProfileID) || null,
        "BusinessContinuityPlanID": Number(localStorage.getItem("BusinessContinuityPlanID")),
        "SectionID": 1,
        "BusinessFunctionID": Number(localStorage.getItem("BusinessFunctionID")),
        "BusinessDescription": this.businessFunctionProfile.value.description,
    ...
    const criticalData   = localStorage.getItem('criticalProcess');

    const RespData = [{
      CriticalBusinessActivities : criticalData,
      Customers                  : customersData
    }]

    let hasChanges   = this.isDataChanged(this.payloadData, RespData[0]);
    this.checkBusinessProfileDataChanges.emit(hasChanges);
    return hasChanges;
  }

  private isDataChanged(newData: any, oldData: any): boolean {
    let CustomersData = JSON.parse(oldData?.Customers);
    let BusinessData  = JSON.parse(oldData?.CriticalBusinessActivities);

    BusinessData = BusinessData.map((nn:any) => {
      return {
          ActivityID  : nn.ActivityID,            
          ActivityName: nn.Description, 
      };
    });

    CustomersData = CustomersData.map((nn:any) => {
      return {
        CustomerID    : nn.CustomerID,            
        CustomerName  : nn.Description, 
        IsInternal    : nn.Affiliate == "Internal" ? true : false
      };
    });
    ...
    if((newData.BusinessDescription != this.savedData[0].BusinessDescription) || (newData.BusinessServices != this.savedData[0].BusinessServices) ) {
      return true;
    }

    if (this.isArrayChanged(newData.CriticalBusinessActivities, BusinessData, ['ActivityID', 'ActivityName'])) {
      return true;
    }

    if (this.isArrayChanged(newData.Customers, CustomersData, ['CustomerID', 'CustomerName', 'IsInternal'])) {
      return true;
    }

    if (this.isArrayChanged(newData.ProfilingQuestions, this.savedData[0].ProfilingQuestions, ['ProfilingQuestionID', 'ProfilingAnswer'])) {
      return true;
    }
  ...
  private isArrayChanged(newArray: any[], oldArray: any[], keysToCheck: string[]): boolean {

    if (!newArray || !oldArray) return true;

    if (newArray.length !== oldArray.length) return true; 

    for (let i = 0; i < newArray.length; i++) {
      let newItem = newArray[i];
      let oldItem = oldArray[i];
