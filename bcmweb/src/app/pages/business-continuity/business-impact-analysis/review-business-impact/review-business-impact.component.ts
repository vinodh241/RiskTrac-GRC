...
import { StaffContactDetailsComponent } from './staff-contact-details/staff-contact-details.component';
import { RecoveryProcessComponent } from './recovery-process/recovery-process.component';

@Component({
  selector: 'app-review-business-impact',
  templateUrl: './review-business-impact.component.html',
  styleUrls: ['./review-business-impact.component.scss']
})
export class ReviewBusinessImpactComponent  implements AfterViewInit {
  ...
  nextFunc : boolean = false;
  IsFAwithBCM:any

  @ViewChild(BusinessFunctionProfileComponent)  profileComponent!: BusinessFunctionProfileComponent;
  @ViewChild(ProcessActivitiesDetailsComponent) processComponent!: ProcessActivitiesDetailsComponent;
  @ViewChild(DependenciesComponent) dependencyComponent!: DependenciesComponent;
  @ViewChild(ImpactAssessmentComponent) impactAssessmentComponent!: ImpactAssessmentComponent;
  @ViewChild(RiskMitigationComponent) riskMitigationComponent!: RiskMitigationComponent;
  @ViewChild(ResourceRequirementsComponent) resourceRequirementsComponent!: ResourceRequirementsComponent;
  @ViewChild(StaffContactDetailsComponent) staffContactDetailsComponent!: StaffContactDetailsComponent;
  @ViewChild(RecoveryProcessComponent) recoveryProcessComponent!: RecoveryProcessComponent;
  editRecovery:any
  constructor(
    ...
    this.authService.activeSubTab$.next("business-impact-analysis");
  }
  ...
  async previousSection(id: any) {
    this.nextFunc = false;
    if (this.selectedSetions == 2) {
      let resp = await this.processComponent.compareProcessActivityData();
      if(!resp) this.selectedSetions = 1
    } else if (this.selectedSetions == 3) {
      let resp = await this.dependencyComponent.compareDependencyData();
      if(!resp) this.selectedSetions = 2
    } else if (this.selectedSetions == 4) {
      let resp = await this.impactAssessmentComponent.compareImpactAssessmentData();
      if(!resp) this.selectedSetions = 3;
    } else if (this.selectedSetions == 5) {
      let resp = await this.riskMitigationComponent.compareRiskMitigationData();
      if(!resp) this.selectedSetions = 4;
    } else if (this.selectedSetions == 6) {
      let resp = await this.resourceRequirementsComponent.compareResourceRequirementsData();
      if(!resp) this.selectedSetions = 5;
    } else if (this.selectedSetions == 7) {
      let resp = await this.staffContactDetailsComponent.comparestaffContactDetailsData();
      if(!resp) this.selectedSetions = 6;
    } else if (this.selectedSetions == 8) {
      let resp = await this.recoveryProcessComponent.compareRecoveryProcessData();
      if(!resp) this.selectedSetions = 7;
    }
  }

  async nextSection(id: any) {
    this.nextFunc = true;
    if (this.selectedSetions == 1) {
      let resp = await this.profileComponent.compareProfilingQuestionsData();
      if(!resp) this.selectedSetions = 2;
      ...
      let resp = await this.staffContactDetailsComponent.comparestaffContactDetailsData();
      if(!resp) this.selectedSetions = 8;
      this.disableNext = true
      this.selectedSetions = 8
    }
  }

  businessProfileDataChanges(event:any) {
    if(event) this.confirmationDialog();
  }

  processActivityDataChanges(event:any) {
    if(event) this.confirmationDialog();
  }

  dependencyDataChanges(event:any) {
    if(event) this.confirmationDialog();
  }

  impactAssessmentDataChanges(event:any) {
    if(event) this.confirmationDialog();
  }

  riskMitigationDataChanges(event:any) {
    if(event) this.confirmationDialog();
  }

  resourceRequirementsDataChanges(event:any) {
    if(event) this.confirmationDialog();
  }

  staffContactChanges(event:any) {
    if(event) this.confirmationDialog();
  }

  recoveryProcessChanges(event:any) {
    if(event) this.confirmationDialog();
  }

  confirmationDialog() {
    let WorkFlowStatusID :any = localStorage.getItem('WorkFlowStatusID')?.toString()
    if(WorkFlowStatusID != 2 && WorkFlowStatusID != 7) {
      console.log('✌️WorkFlowStatusID --->', WorkFlowStatusID);
      this.selectedSetions = this.nextFunc ? (Number(this.selectedSetions) + 1) : (Number(this.selectedSetions) - 1);
      return;
    }
    const confirm = this.dialog.open(ConfirmDialogComponent, {
      ...
      data: {
        title: 'Confirmation',
        content: 'Unsaved changes detected. If you proceed, these changes will be lost. Would you like to proceed?',
      },
    });
  ...
  navigateToListing() {
    this.route.navigate(['/business-impact-analysis/business-impact-analysis']);
  }
  ...
  processData(event: boolean) {
    ...
    console.log('this.dataProcessSaved: '+this.dataProcessSaved)
  }
  ...
  editCheck(event: boolean) {
    this.editRecovery = event;
    if (this.editRecovery == true) {
      this.displayButton()
    }
  }
  ...
  workflowDetails(event: any) {
    ...
    this.IsFAwithBCM = event.IsFAwithBCM;
  }

  displayButton() {
    if (this.IsFAwithBCM == false || this.IsBCC == true && (this.currentStatusID == 2 || this.currentStatusID == 7)) {
