...
@Component({
  selector: 'app-business-impact-listing',
  templateUrl: './business-impact-listing.component.html',
  styleUrls: ['./business-impact-listing.component.scss']
})
export class BusinessImpactListingComponent {
  ...
  constructor(
    ...
    this.businessContinuityService.gotMaster.subscribe((value) => {
      this.dataSource = this.businessContinuityService.masterData?.BusinessContinuityPlansList || [];
  ...
  initiateBCP() {
    const dialog = this.dialog.open(InitiateBiaExerciseComponent, {
      disableClose: true,
      maxWidth: '63vw',
  ...
  applyFilter(event: Event) {
    ...
    this.dataSource = addIndex(JSON.parse(JSON.stringify(searchBy(filterValue, searchFields, this.businessContinuityService.masterData['BusinessContinuityPlansList'], this.customFilterFields, this.assessmentStatusIds))), false)
  }
  ...
  select(item?: any) {
    ...
    this.router.navigate(['business-impact-analysis/review-business-impact'])
  }
  ...
  openConsolidatedReport() {
    ...
    const dialog = this.dialog.open(ConsolidatedReportComponent, {
      ...
      data: {
        popupHeader: 'Choose one or more business continuity plans from the published BIA reports listed below. The selected ones will be consolidated and summarised in the report.',
        notes: this.exportedFileLimit != null ? `Maximum ${this.exportedFileLimit} report(s) can be generated at once` : '',
        exportFileLimit: this.exportedFileLimit,
        tableData: [
          {columnDefs: 'BusinessFunction', header: 'Business Function', property: 'BusinessFunctionName', columnStyle: 'w-10', sort: false},
          {columnDefs: 'Group', header: 'Business Group', property: 'BusinessGroup', columnStyle: 'w-10', sort: false},
          {columnDefs: 'MTPD', header: 'MTPD', property: 'MTPDValue', columnStyle: 'w-10', sort: false},
          {columnDefs: 'RTO', header: 'RTO', property: 'RTOValue', columnStyle: 'w-10', sort: false},
          {columnDefs: 'RPO', header: 'RPO', property: 'RPOValue', columnStyle: 'w-10', sort: false},
          // {columnDefs: 'BCC', header: 'BCC', property: 'BCCName', columnStyle: 'w-10 text-center', sort: false},
          {columnDefs: 'CompletionDate', header: 'Review Completion', property: 'FinalDate', columnStyle: 'w-10 text-center', sort: false},
          {columnDefs: 'BIARating', header: 'BIA Rating', property: 'BIARating', columnStyle: 'w-10 text-center', sort: false},
        ],
