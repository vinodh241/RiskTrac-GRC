...
class RiskCategoryBl{
    ...
    async addRiskCategory(request, response){
        ...
        try {
            ...
            binds.CorporateObjectiveID = request.body.corporateObjectiveID || 0;
            binds.createdBy = userIdFromToken || "";
            binds.userId = userIdFromToken;
            binds.userName = userNameFromToken;
            binds.refreshedToken = refreshedToken;
            ...
            if (appValidatorObject.isStringUndefined(request.body.category) || appValidatorObject.isStringNull(request.body.category)
                || appValidatorObject.isStringEmpty(request.body.category)) {
                validationMessage.push('Category');
            }

            if (request.body.corporateObjectiveID === undefined || request.body.corporateObjectiveID === null
                || request.body.corporateObjectiveID === '' || isNaN(request.body.corporateObjectiveID)
                || parseInt(request.body.corporateObjectiveID) === 0) {
                validationMessage.push('Corporate Objective');
            }
