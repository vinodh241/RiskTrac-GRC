...
module.exports = class UtilityApp {
    ...
    getAccountNameFromToken(token) {
        ...
        try {
            ...
            logger.log('error', 'UtilityApp(ORM) : getAccountNameFromToken : Error details :' + error);
    ...
    async getAccountGUIDFromToken(token) {
        ...
        try {
            decodedToken = JWT.decode(token, { complete: CONSTANT_FILE_OBJ.APP_CONSTANT.TRUE });      // Decoding token
            // logger.log('info', 'UtilityApp(ORM) : getAccountGUIDFromToken :  ORM : DECODED_TOKEN : ' + JSON.stringify(decodedToken));
            ...
            // logger.log('info', 'UtilityApp(ORM) : getAccountGUIDFromToken :  ORM : ACCOUNT_GUID_FROM_NAME : ' + JSON.stringify(ACCOUNT_GUID_FROM_NAME));
            ...
            // logger.log('info', 'UtilityApp(ORM) : getAccountGUIDFromToken :  ORM : ACCOUNT_TOKEN_OBJECT : ' + JSON.stringify(accountGUIDFromTokenObj));
            return accountGUIDFromTokenObj;

        } catch (error) {
            logger.log('error', 'UtilityApp(ORM) : getAccountGUIDFromToken : Error details :' + error);
            accountGUIDFromTokenObj.error = CONSTANT_FILE_OBJ.APP_CONSTANT.TRUE;
