...
module.exports = async function validateUpdateToken(request, response, next){
    try {
        ...
        let moduleAccessData = GET_INFO_USER_LOGIN.recordset[CONSTANT_FILE_OBJ.APP_CONSTANT.TWO];

        let roleId          = usersLoginData.filter(ele => ele.UserGUID == userIdFromToken).map(ele => ele.RoleID);

        moduleAccessData = moduleAccessData.filter(n => n.ModuleAbbrevation === APP_CONFIG.ALLOWED_MODULE_ABBREVIATION.ABBREVIATION);
        ...
        // to check whether logged in user is having access to the module. - START
        let userModuleAccessData    = GET_INFO_USER_LOGIN.recordset[CONSTANT_FILE_OBJ.APP_CONSTANT.TWO];
        userModuleAccessData        = userModuleAccessData && userModuleAccessData.length ? userModuleAccessData.filter(n => n.Abbreviation === APP_CONFIG.ALLOWED_MODULE_ABBREVIATION.ABBREVIATION) : [];

        if (!userModuleAccessData.length) {
            logger.log('error', 'User Id : ' + userIdFromToken + ' : ValidateUpdateTokenMiddleware : validateUpdateToken : Execution end. : Error Detail : user is not allowed to access the module.');
            return response.status(CONSTANT_FILE_OBJ.APP_CONSTANT.TWO_HUNDRED).json(unsuccessfulResponse(CONSTANT_FILE_OBJ.APP_CONSTANT.NULL, CONSTANT_FILE_OBJ.APP_CONSTANT.TOKEN_EXPIRED, MESSAGE_FILE_OBJ.MESSAGE_CONSTANT.UNAUTHORIZED_ACCESS));
        }
        //to check whether logged in user is having access to the module. - END
                    ...
                    if (COMMON_DB_RESPONSE.procedureMessage === CONSTANT_FILE_OBJ.APP_CONSTANT.INVALID_SESSION) {
                        ...
                        */
                        if (validateFunction != CONSTANT_FILE_OBJ.APP_CONSTANT.ZERO) {
                            const GET_INFO_FOR_AUTHORIZED = await commonDBObject.getInfoFunctionAuthorized(userIdFromToken, userNameFromToken, accountGUIDFromToken);
