...
module.exports = class EmailNotification {
     ...
     * @param {*} data 
     */
    async formatDataForSendEmail(userIdFromToken, userNameFromToken, emailTemplateObj, templateMasterData, toccEmails,attachments) {
        try {
            ...
            const NOTIFICATION_RESPONSE = await this.sendEmailNotification(userIdFromToken,userNameFromToken,  emailTemplateObj, templateMasterData, toccEmails,attachments);
     ...
     */
    async sendEmailNotification(userIdFromToken, userNameFromToken, emailTemplateObj, templateMasterData, toccEmails,attachments) {
        try {
        notificationlogger.log('info', 'User Id : '+ userIdFromToken +' : EmailNotification : sendEmailNotification : Execution started.');
            ...
            const preparedTemplate              = templateObject.prepareTemplates(userIdFromToken, emailTemplateObj, templateMasterData);
            ...
            NOTIFICATION_MASTER.attachments     = attachments != CONSTANT_FILE_OBJ.APP_CONSTANT.NULL ? attachments:CONSTANT_FILE_OBJ.APP_CONSTANT.NULL;
